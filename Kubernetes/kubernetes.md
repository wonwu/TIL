...
## PV PVC
호스트에서 분리할 수 있는 '외부 스토리지 형태의 볼륨'
- 호스트와 데이터 볼륨의 결합이 느슨함
- 볼륨이 새로 배치된 호스트에 자동할당됨
...
- 추상화된 논리 리소스
- 용량 동적 확보 
- 데이터를 볼륨에 넣고나면, 파드를 정지하거나 실행해도 애플리케이션 상태 

## Statefulset
- 데이터를 계속 유지하는 상태가 있는 (stateful) 애플리케이션을 관리하는데 적합한 리소스
- 일련번호(식별자)는 파드를 재생성해도 유지됨
- 스케일링할 때도 식별자의 일련번호가 계속 이어짐
- 파드의 데이터를 재생성 전과 똑같이 복원 

| deployment |  statefulset   |
------|------------------------
 상태가 없는 (statelss) 애플리케이션을 배포하는데 적합 | 데이터를 계속 유지하는 상태가 있는 (stateful) 애플리케이션을 관리하는데 적합
 무작위로 생성된 식별자 부여됨 | 일련번호가 붙는 유일한 식별자를 붙여 파드를 생성
...
- 스케일아웃 가능 (파드 여러개 병렬 실행)


## Cronjob
- 스케줄을 지정해 '정기적'으로 파드 실행
- 백업용
- CronTab


## Secret
- 기밀 정보 문자열을 Base64인코딩으로 
